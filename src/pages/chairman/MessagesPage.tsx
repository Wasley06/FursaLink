...
export default function ChairmanMessagesPage() {
  ...
  useEffect(() => {
    const run = async () => {
      if (!profile) return;
      const [inbox, sent, controllers, developers] = await Promise.all([
        ...
        getDocs(query(collection(db, 'users'), where('role', '==', 'controller'), limit(100))),
        getDocs(query(collection(db, 'users'), where('role', '==', 'developer'), limit(50))),
      ]);
      setMessages([
      ...
      developers.docs.forEach((d) => people.push({ id: d.id, ...(d.data() as any) } as UserProfile));
    ...
    <div className="space-y-6">
      <div className="premium-card">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-navy">Internal Communications</h1>
            <p className="text-sm text-muted font-medium">Executive messaging with audit visibility.</p>
          </div>
        </div>
      </div>
