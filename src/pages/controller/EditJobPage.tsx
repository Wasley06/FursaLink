...
export default function EditJobPage() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await logAudit('job:update', { jobId: job.id });
  ...
  if (!job) {
    return <div className="premium-card text-sm text-muted italic">Job not found.</div>;
  }
      ...
      <form onSubmit={save} className="premium-card space-y-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setJob((p) => (p ? { ...p, occupation: e.target.value } : p))}
          />
        </div>
        <div>
          ...
          <textarea
            ...
            onChange={(e) => setJob((p) => (p ? { ...p, description: e.target.value } : p))}
          />
        </div>
        <div>
          ...
          <textarea
            ...
            onChange={(e) => setJob((p) => (p ? { ...p, qualifications: e.target.value } : p))}
          />
        </div>
