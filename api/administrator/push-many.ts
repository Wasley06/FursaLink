...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const rawIds: any[] = Array.isArray(body?.candidateIds) ? (body.candidateIds as any[]) : [];
    const candidateIds: string[] = rawIds.map((x) => String(x || '').trim()).filter(Boolean);
    const chairmanRemarks = String(body?.chairmanRemarks || '');
    if (candidateIds.length === 0) return json(res, 400, { error: 'missing_candidate_ids' });
    ...
    const unique: string[] = Array.from(new Set(candidateIds.map((x) => String(x)))).slice(0, 500);
    const snaps = await Promise.all(unique.map((id) => db.collection('users').doc(String(id)).get()));
